...
import { demoNotice, workspace } from "@/lib/seed";
      ...
      <div className="mx-auto flex max-w-7xl gap-6 px-4 py-4 lg:px-8">
        <aside className="sticky top-4 hidden h-[calc(100vh-2rem)] w-72 shrink-0 flex-col rounded-[2rem] border border-white/80 bg-white/80 p-5 shadow-soft backdrop-blur lg:flex">
          ...
          <div className="mt-auto rounded-3xl bg-plum p-4 text-white">
            <p className="text-sm opacity-80">目前角色</p><p className="font-semibold">{roleLabel("owner")}</p>
            <p className="mt-3 text-xs leading-5 opacity-75">{demoNotice}</p>
          </div>
        </aside>
        <section className="min-w-0 flex-1">
          <header className="mb-5 flex flex-col gap-4 rounded-[2rem] bg-white/70 p-4 shadow-soft backdrop-blur sm:flex-row sm:items-center sm:justify-between">
            <div><p className="text-sm font-semibold text-rose">{workspace.businessHours}</p><h1 className="text-2xl font-bold tracking-tight text-plum sm:text-3xl">{title}</h1><p className="mt-1 text-sm text-ink/60">{subtitle}</p></div>
            <div className="flex gap-2"><Link className="mobile-tap rounded-2xl bg-plum text-sm font-semibold text-white" href="/appointments">新增預約</Link><Link className="mobile-tap rounded-2xl bg-white text-sm font-semibold text-plum" href="/checkout">快速結帳</Link></div>
          </header>
          {children}
        </section>
      </div>
      <nav className="fixed inset-x-3 bottom-3 z-20 grid grid-cols-5 rounded-[1.5rem] border border-white bg-white/95 p-2 shadow-soft backdrop-blur lg:hidden">
        {nav.slice(0, 5).map((item) => <Link key={item.href} href={item.href} className="flex flex-col items-center gap-1 rounded-2xl px-2 py-2 text-[11px] font-semibold text-plum"><item.icon size={20} />{item.label}</Link>)}
      </nav>
    </main>
  );
}
